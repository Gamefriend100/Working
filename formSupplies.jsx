...
const Form = ({ item, mode }) => {
  ...
  const { initCategories, isFechingCategories, supplies, setSupplies } = UseKitContext();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = item
        ? `/intranet/supplies/edit/${item.value}`
      ...
      const newLocalData = item
        ? supplies.map((s) => (s.value === item.value ? data.supply : s))
  ...
  const handleDelete = async () => {
    try {
      await appApi.delete(`/intranet/supplies/delete/${item.value}`, {
        headers: { Authorization: `Bearer ${authUserToken}` },
      });

      setSupplies(supplies.filter((s) => s.value !== item.value));
